...
MMEvents.createProcesses((event) => {
  ...
  const tier1FarmlandConfigs = [
    ...
    {
      farmland: 'mysticalagriculture:prudentium_farmland',
      ticks: 1400,
      output: 32,
    },
    {
      farmland: 'mysticalagriculture:tertium_farmland',
      ticks: 1300,
      output: 48,
    },
    {
      farmland: 'mysticalagriculture:imperium_farmland',
      ticks: 1200,
      output: 64,
    },
    {
      farmland: 'mysticalagriculture:supremium_farmland',
      ticks: 1000,
      output: 96,
    },
    {
  ...
  createMultiSeedRecipes(event, tier2Seeds, tier2FarmlandConfigs, 4);
  ...
  createMultiSeedRecipes(event, tier4Seeds, tier4FarmlandConfigs, 4);
  ...
  createMultiSeedRecipes(event, tier5Seeds, tier5FarmlandConfigs, 4);
...
function createMultiSeedRecipes(
  ...
  for (let i = 0; i < seeds.length; i += maxSeedsPerRecipe) {
    ...
    farmlandConfigs.forEach((config) => {
      ...
      const recipeId = `mm:essenceforge_${chunk
        .map((s) => s.replace(':', '_'))
        .join('_')}_${farmland.replace(':', '_')}`;
      const recipe = event
        .create(recipeId)
        .structureId('mm:essenceforge_structure_on2')
        .ticks(ticks)
        .input({
        ...
        .input({
          type: 'mm:input/consume',
          chance: 0.0,
          ingredient: { type: 'mm:item', item: farmland, count: 1 },
        })
        .input({
          type: 'mm:input/consume',
          ingredient: {
      ...
      chunk.forEach((seed) => {
        ...
        recipe.output({
          ...
          ingredient: { type: 'mm:item', item: essence, count: config.output },
        });
      });
    });
  }
...
function createTier6SeedRecipes(event, seeds) {
  ...
  seeds.forEach((seed) => {
    ...
    event
      ...
      .output({
        type: 'mm:output/simple',
        ingredient: { type: 'mm:item', item: essence, count: 128 },
      });
  });
}
